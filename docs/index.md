<!-- Mirror of the repository README with docs-site link adjustments. -->
...
If a task should run continuously, the loop belongs inside the task function. If you need raw ring access instead of framed stream helpers, the lower-level `Manager`, `RingSpec`, and `TaskSpec` APIs are still available.
...
The benchmark command set and output conventions are documented in [Benchmark Guide](benchmarks.md). Install `.[benchmarks]` for the full benchmark and comparison set.
